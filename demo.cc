...
int TSS(BitBoard my, BitBoard opp, BitBoard Tzone, int depth) {
    ...
    while (cTzone && !check) {
        ...
        if (s_h & 0x30000) {
            check = TSS(my, opp & Cboard_h[(pos << 5) | (s_h & 0x1F)],
                        Tzone | Tboard_h[(s_h & 0x3F00) | pos] |
                            Tboard_b[(s_b & 0x3F00) | pos] |
                            Tboard_v[(s_v & 0x3F00) | pos] |
                            Tboard_s[(s_s & 0x3F00) | pos],
                        depth - 1);
        } else if (s_b & 0x30000) {
            check = TSS(my, opp & Cboard_b[(pos << 5) | (s_b & 0x1F)],
                        Tzone | Tboard_h[(s_h & 0x3F00) | pos] |
                            Tboard_b[(s_b & 0x3F00) | pos] |
                            Tboard_v[(s_v & 0x3F00) | pos] |
                            Tboard_s[(s_s & 0x3F00) | pos],
            ...
            check = TSS(my, opp & Cboard_v[(pos << 5) | (s_v & 0x1F)],
                        Tzone | Tboard_h[(s_h & 0x3F00) | pos] |
                            Tboard_b[(s_b & 0x3F00) | pos] |
                            Tboard_v[(s_v & 0x3F00) | pos] |
                            Tboard_s[(s_s & 0x3F00) | pos],
            ...
            check = TSS(my, opp & Cboard_s[(pos << 5) | (s_s & 0x1F)],
                        Tzone | Tboard_h[(s_h & 0x3F00) | pos] |
                            Tboard_b[(s_b & 0x3F00) | pos] |
                            Tboard_v[(s_v & 0x3F00) | pos] |
                            Tboard_s[(s_s & 0x3F00) | pos],
...
int ABSearch(BitBoard my, BitBoard opp, BitBoard myTzone, BitBoard oppTzone,
             BitBoard moves, BitBoard Rzone, int alpha, int beta, int depth,
             int score) {
    ...
    while (mov) {
        ...
        uint64_t s_v = state[_pext_u64(feat, pext_v)];
        uint64_t s_s = state[_pext_u64(feat, pext_s)];
        BitBoard newTzone = myTzone | Tboard_h[(s_h & 0x3F00) | pos] |
                            Tboard_b[(s_b & 0x3F00) | pos] |
                            Tboard_v[(s_v & 0x3F00) | pos] |
                            Tboard_s[(s_s & 0x3F00) | pos];
        ...
        int value = -ABSearch(opp, my, oppTzone, newTzone, moves.mind(pos),
                              endRzone, -beta, -alpha, depth - 1, -score);
...
uint64_t ABCaller(BitBoard my, BitBoard opp, BitBoard myTzone,
                  BitBoard oppTzone, BitBoard moves, int alpha, int beta,
                  int depth, int score) {
    if (depth <= 0) return score;
...
uint64_t Input(char c, int n) { return (15 - n) * 16 + c - 'A' + 1; }
struct Game {
    BitBoard black, white, moves, blackTzone, whiteTzone;
    BitBoard black_record[256], white_record[256], moves_record[256],
        blackTzone_record[256], whiteTzone_record[256];
...
void Game::Move(uint64_t pos) {
    ...
    if (size % 2 == 0) {
        ...
        uint64_t s_v = state[_pext_u64(feat, pext_v)];
        uint64_t s_s = state[_pext_u64(feat, pext_s)];
        blackTzone = blackTzone | Tboard_h[(s_h & 0x3F00) | pos] |
                     Tboard_b[(s_b & 0x3F00) | pos] |
                     Tboard_v[(s_v & 0x3F00) | pos] |
                     Tboard_s[(s_s & 0x3F00) | pos];
    } else {
        ...
        uint64_t s_v = state[_pext_u64(feat, pext_v)];
        uint64_t s_s = state[_pext_u64(feat, pext_s)];
        whiteTzone = whiteTzone | Tboard_h[(s_h & 0x3F00) | pos] |
                     Tboard_b[(s_b & 0x3F00) | pos] |
                     Tboard_v[(s_v & 0x3F00) | pos] |
                     Tboard_s[(s_s & 0x3F00) | pos];
    }
...
void Game::Play(int depth) {
    ...
    if (size % 2 == 0) {
        ...
        for (int i = 1; i <= depth; i++) {
            pos = ABCaller(black, white, blackTzone, whiteTzone, moves, -30000,
                           30000, i, score);
        }
    } else {
        pos = 0;
        ...
        if (pos == 0) {
            for (int i = 1; i <= depth; i++) {
                pos = ABCaller(white, black, whiteTzone, blackTzone, moves,
                               -30000, 30000, i, score);
            }
        }
    }
